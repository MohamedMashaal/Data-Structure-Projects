...
public class ExpressionEvaluator implements IExpressionEvaluator {
	...
	public String infixToPostfix(String expression) {
		...
		if (t.charAt(0) == ')' || t.charAt(0) == '-' || t.charAt(0) == '+' || t.charAt(0) == '*' || t.charAt(0) == '/'
				|| t.charAt(0) == '!')
			throw new RuntimeException("Error");

		for (int i = 0; i < t.length() - 1; i++)
			if ((t.charAt(i) == '+' || t.charAt(i) == '*' || t.charAt(i) == '/' || t.charAt(i) == '-')
					&& (t.charAt(i + 1) == '+' || t.charAt(i + 1) == '/' || t.charAt(i + 1) == '*'
							|| t.charAt(i + 1) == '-')) {
				throw new RuntimeException("Error");
			}
		if (t.charAt(t.length() - 1) == '+' || t.charAt(t.length() - 1) == '-' || t.charAt(t.length() - 1) == '*'
				|| t.charAt(t.length() - 1) == '/') {
			throw new RuntimeException("Error");
		}
